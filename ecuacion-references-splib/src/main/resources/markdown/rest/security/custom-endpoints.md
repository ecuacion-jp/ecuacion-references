...
catch-all for anything under `/api/**` that is not `/api/public/**` or `/api/key/**`.

## Adding your own security policy

To expose an endpoint under a path other than the two built-in prefixes, or to apply a different
...
`/api/public/**` (8), `/api/key/**` (9), and finally the `/api/**` deny-all (10) chains, in that
order.

## Reserved orders

`ecuacion-splib-rest` uses `@Order(8)`, `@Order(9)`, and `@Order(10)` for
[Public Endpoints](/public/showMarkdown/page?id=rest/security/public-endpoints&lang=en),
[API Key Authentication](/public/showMarkdown/page?id=rest/security/api-key/overview&lang=en), and the
