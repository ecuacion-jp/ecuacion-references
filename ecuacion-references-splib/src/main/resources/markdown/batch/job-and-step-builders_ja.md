`SplibAppParentBatchConfig`（[セットアップ](/public/showMarkdown/page?id=batch/setup&lang=ja) 参照）は、
...
- `.listener(jobExecutionListener)` — コンストラクターに渡された `SplibJobExecutionListener`。
  [ロギング](/public/showMarkdown/page?id=batch/logging&lang=ja) を参照してください。
...
[ロギング](/public/showMarkdown/page?id=batch/logging&lang=ja) で説明するログ出力と
[例外処理](/public/showMarkdown/page?id=batch/exception-handling&lang=ja) で説明する例外処理を自動的に得られます。
