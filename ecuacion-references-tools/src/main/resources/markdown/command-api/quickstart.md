...
By default, `GET` is disabled and `POST` requires an `apiKey` (see [Access Control](/public/showMarkdown/page?id=command-api/config&lang=en#access-control)). For this local quickstart, add the following to `application.properties` (placed as described in [Configuration Files](/public/showMarkdown/page?id=command-api/config&lang=en)) and restart the app:

```properties
jp.ecuacion.tool.command-api.allow-insecure-access=true
```

(For production use, keep this unset/`false` and call the API with `POST` + `apiKey` instead — see [API Spec](/public/showMarkdown/page?id=command-api/api-spec&lang=en).)
