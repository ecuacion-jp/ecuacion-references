# code-generator-web Configuration Files
...
> **Note:** The properties below have no default values. If any of `spring.mail.host` / `spring.mail.username` / `spring.mail.password` / `jp.ecuacion.splib.mail.address-csv-on-system-error` is left unset, no mail notification is sent on error (i.e., the default behavior is "no mail is sent").
